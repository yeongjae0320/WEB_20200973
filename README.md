...
-1. 비속어(욕) 3개 필터링 하는 코드를 추가하였다. search.js 에서 
forbiddenWords 배열로 금지어1, 금지어2, 금지어3 의 문자열을 선언했고, 
search_message() 함수를 실행할 때 금지어를 포함하고 있으면 
"검색어가 제한 단어에 해당됩니다. 다른 검색어를 입력해주세요." 라는 
메시지 창을 띄우는 것으로 구현했다.

## 7주차 문제 및 추가구현

-1. 검색어 목록을 10개로 제한을 두기 위해 var MAX_SEARCH_COUNT = 10; 으로 
선언했다. 만약에 빈 배열의 길이가 최대 검색어 개수보다 길거나 같으면 
"최대 검색어 개수를 초과하였습니다." 라는 메시지 창을 띄우고, 
.splice() 을 사용하여 배열의 첫 번째 값, 즉, 맨 앞의 값을 삭제하게 했다.

-2. 기존 카운트다운 하는 팝업창에서 카운트다운은 숫자로만 표기가 되어 사용자가 이것이 
무슨 의미인지 바로 파악을 못하는 문제점이 있다. 그렇기에 "남은 시간은 @초 입니다." 의 
형태로 보여주기 위해 문자열 "남은 시간은 " + close_time2 + "ch dlqslek." 으로 
수정했다.
...
문제의 요구사항은 로그인 유지 시간을 5분으로 유지하는 거였지만, 
실제로 코드가 정상적으로 
실행되는지 확인하기 위해 조건을 변경하여 제한 시간을 30초로 설정하였다. 
...
"Session_Storage_object" 와 
"Session_Storage_encrypted" 을 제거하는 것으로 구현하였다.

-3. login_fail() 함수와 login_fail 쿠키를 만들었다. 
login_fail 쿠키가 존재하지 않으면 초기값으로 1로 
설정하고 쿠키가 존재하면 현재 값에 1을 더하는 로그인 및 로그아웃 쿠키와 동일한 로직을 
사용하였다. 로그인 실패 횟수가 3번 이면 "로그인을 3번 실패하셨습니다. 페이지를 
닫겠습니다. 새 창을 여세요." 메시지 창을 띄우고, self.close(); 를 사용하여 
현재 페이지를 닫게 구현했다.
...
첫 번째로, 좌측 상단과 우측 하단에 문자 내용을 입력하였고 
화면 중앙에 구글 이미지를 넣었다. 
그리고 중앙 구글 이미지 아래에 검색창 문자열과 박스를 추가했다. 추가로 
검색 버튼을 눌렀을 때 검색을 수행한다는 메시지 창을 띄우기 위해 
button 태그의 onclick 속성 값으로 alert() 를 사용하였다.

 기존 head 태그 사이에 메타데이터, 페이지 세부 정보를 표현해주었고 
구글 이미지를 클릭하면 현재 웹 메인 페이지에 접속하게 
a 태그의 href 속성에 링크를 추가하여 수정해주었다. 링크는 index.html 으로 설정하였다. 
그리고 중앙 검색창 문자열과 박스 아래에 표를 삽입했다.

 부트스트랩 사용을 위해 html 코드 head 태그 부분에 
link 태그의 href 속성, script 태그의 src 속성 
등을 추가하였다. 또, 부트스트랩에서 제공하는 
네비게이션 바 링크의 코드를 이용하여 기존 index.html 에 추가했다. 이때, 좌측 상단에 
있던 문자 내용은 삭제해주었다. 드롭다운 메뉴를 추가해주었고, 각 메뉴를 클릭했을 때 
사이트를 연결해서 접속시켜주기 위해 비어있던 링크들을 다 채워주었다. 
...
위 여백 10, 왼쪽 여백 10 크기의 팝업창 html 페이지를 열도록 구현했다. 
...
메인 페이지의 구글 이미지에 마우스를 올리면 이미지가 바뀌는 hover 기능을 추가해주었다. 
구글 이미지 2개를 더 추가하여 
onmouseover() 와 onmouseout() 을 사용하여 메인 이미지에 변화를 주었다.
또, 문자열 검사 기능을 넣었는데, 
...
js 내장 함수인 setTimeout(함수, 시간, 인수); 을 사용하였다. 
그리고 홈페이지에서 검색창에 입력한 문자열을 배열에 저장하는 코드를 구현했다. 
var search_array = []; 으로 배열을 비어있는 상태로 초기화하고, 함수의 검색 수행 코드 
아래에 search_array.push(); 을 사용하여 배열에 검색어를 추가했다. 
...
login.js 파일을 새로 만들었다. 버튼을 클릭하면 form을 제출할 수 있게 
